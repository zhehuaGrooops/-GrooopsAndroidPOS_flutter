...
class USBPrinterAdapter private constructor() {
    ...
    fun init(reactContext: Context?) {
        ...
        mPermissionIndent = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            PendingIntent.getBroadcast(ctx, 0, Intent(ACTION_USB_PERMISSION), PendingIntent.FLAG_MUTABLE)
        } else {
            PendingIntent.getBroadcast(ctx, 0, Intent(ACTION_USB_PERMISSION), 0)
        }
            ...
            Thread(object : Runnable {
                override fun run() {
                    synchronized(printLock) {
                        ...
                        val b: Int = connection.bulkTransfer(
                            endPoint,
                            bytes,
                            bytes.size,
                            100000
